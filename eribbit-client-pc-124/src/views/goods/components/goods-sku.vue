    ...
    <dl v-for="item in goods.specs" :key="item.id">
      ...
      <dd>
        <template v-for="val in item.values" :key="val.name">
          <img :class="{selected:val.selected}" @click="clickSpecs(item,val)" v-if="val.picture" :src="val.picture" :title="val.name" :alt="val.name">
  ...
  setup (props, { emit }) {
    ...
    const clickSpecs = (item, val) => {
      ...
      if (selectedArr.length === props.goods.specs.length) {
        ...
        const sku = props.goods.skus.find(sku => sku.id === skuIds[0]) // 传递
        emit('change', {
          skuId: sku.id,
          price: sku.price,
